...
package com.appsnado.haippNew.browser.activity
...
import com.appsnado.haippNew.BrowsingHistory.AddUrl
import com.appsnado.haippNew.AppTheme
import com.appsnado.haippNew.IncognitoActivity
import com.appsnado.haippNew.R
import com.appsnado.haippNew.browser.*
...
import com.appsnado.haippNew.browser.tabs.TabsDesktopView
import com.appsnado.haippNew.browser.tabs.TabsDrawerView
import com.appsnado.haippNew.controller.UIController
import com.appsnado.haippNew.database.Bookmark
import com.appsnado.haippNew.database.HistoryEntry
import com.appsnado.haippNew.database.SearchSuggestion
import com.appsnado.haippNew.database.WebPage
...
import com.appsnado.haippNew.di.*
...
import com.appsnado.haippNew.extensions.*
import com.appsnado.haippNew.html.bookmark.BookmarkPageFactory
import com.appsnado.haippNew.html.history.HistoryPageFactory
import com.appsnado.haippNew.html.homepage.HomePageFactory
import com.appsnado.haippNew.icon.TabCountView
import com.appsnado.haippNew.interpolator.BezierDecelerateInterpolator
import com.appsnado.haippNew.log.Logger
import com.appsnado.haippNew.notifications.IncognitoNotification
import com.appsnado.haippNew.search.SearchEngineProvider
import com.appsnado.haippNew.search.SuggestionsAdapter
import com.appsnado.haippNew.ssl.SslState
import com.appsnado.haippNew.ssl.createSslDrawableForState
import com.appsnado.haippNew.ssl.showSslDialog
import com.appsnado.haippNew.utils.*
import com.appsnado.haippNew.view.*
import com.appsnado.haippNew.view.SearchView
import com.appsnado.haippNew.view.find.FindResults
...
abstract class BrowserActivity : ThemableBrowserActivity(), BrowserView, UIController,
    OnClickListener {
    ...
    private val mListener: ValueEventListener? = null
    // Image
    ...
    private fun initialize(savedInstanceState: Bundle?) {
        ...
        bookmarksView =
                BookmarksDrawerView(this).also(findViewById<FrameLayout>(getBookmarksContainerId())::addView)
    ...
    private fun setNavigationDrawerWidth() {
        val width =
                resources.displayMetrics.widthPixels - dimen(R.dimen.navigation_drawer_minimum_space)
    ...
    private fun showFindInPageControls(text: String) {
        ...
        findViewById<TextView>(R.id.search_query).text =
                resources.getString(R.string.search_in_page_query, text)
        ...
        Palette.from(favicon ?: webPageBitmap!!).generate { palette ->
            ...
            val animation = object : Animation() {
                override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
                    val animatedColor =
                            DrawableUtils.mixColor(interpolatedTime, currentUiColor, finalColor)
                    if (shouldShowTabsInDrawer) {
    ...
    private fun getSearchBarColor(requestedColor: Int, defaultColor: Int): Int =
            if (requestedColor == defaultColor) {
                if (isDarkTheme) DrawableUtils.mixColor(
                        0.25f,
                        defaultColor,
                        Color.WHITE
                ) else Color.WHITE
...
//
//        } else {
//            for (AddUrl in items_list_h) {
//                var string = "https://" + AddUrl.url!! +"/"
//                var string2 = "https://www." + AddUrl.url!!
//                var string3 = "http://" + AddUrl.url!!
//                if (string!!.startsWith(url)) {
//                    ToastUtil.showToast("Block Url")
//                    updaturl = true
//                    break
...
//              }
    ...
    protected fun addItemToHistory(title: String?, url: String) {
        ...
        addviewmodel(url, title)
    }


    open fun printDifference(startDate: Date, endDate: Date) {
    ...
    private fun addviewmodel(url: String, title: String?) {
            var refsignup: DatabaseReference? = null
            var refsignupchild: DatabaseReference? = null
            var database: FirebaseDatabase? = null
            database = FirebaseDatabase.getInstance();
            refsignup = database!!.getReference("WebHistroy")
            refsignupchild = refsignup!!.child("Url")
            addUserChangeListener(refsignupchild!!, url, title!!)
    }
       ...
       myRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                try {
                    ...
                    val stime = tsLong.toString()

                    val prefs = getSharedPreferences(MY_PREFS_NAME, MODE_PRIVATE)
                    val name = prefs.getString("type", null)
                    ...
                    if (name != null) {
                        if (name == "Child") {
                            ...
                            if (data.value != null) {
                                ...
                                val items: ArrayList<AddUrl> =
                                    dataSnapshot.getValue<ArrayList<AddUrl>>(t)!!



                                try {
                                    val last = items.get(items.size - 2)

                                    var size = items.size - 2;
                                    val add = AddUrl()
                                    add.title = last.title
                                    add.starttime = last.starttime
                                    add.endtime = stime
                                    add.url = last.url
...
//                                    items?.add(items.size - 2,add)
                                ...
                                val items: ArrayList<AddUrl> = ArrayList<AddUrl>()
                                val add = AddUrl()
                                add.title = title
                                add.starttime = stime
                                add.endtime = "0"
