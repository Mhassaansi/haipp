...
class ChangepasswordViewModel(services: WebServiceFactory, activity: Activity?) : BaseViewModel<Any>() {
    ...
    fun loadDataNetwork(opass: String?,npass: String?) {
        setIsLoading(true)
        if(opass == "1")
            webservices!!.getservices()!!.changenewpass(userid,npass)!!.enqueue(ResponcesCallback())
          else
            webservices!!.getservices()!!.changepass(userid,opass,npass)!!.enqueue(ResponcesCallback())
    }
...
//                            .registerTyp
//                            eAdapterFactory(LengthArrayTypeAdapterFactory.getLengthArrayTypeAdapterFactory())
//                            .create()
