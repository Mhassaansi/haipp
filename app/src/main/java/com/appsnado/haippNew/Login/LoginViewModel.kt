...
class LoginViewModel(services: WebServiceFactory, activity: Activity?) : BaseViewModel<Any>() {
    ...
    fun loadDataNetwork(userEmail: String?, password: String?,token: String?) {
        setIsLoading(true)
        webservices!!.getservices()!!.login(userEmail, password, "android", token)!!.enqueue(ResponcesCallback())
    }
